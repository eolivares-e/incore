...
**File**: `backend/app/domains/billing/models.py`  
**Estimated Lines**: ~200
...
class Invoice(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("amount_paid >= 0", name="check_invoice_amount_paid_non_negative"),
        CheckConstraint("amount_paid <= amount_due", name="check_invoice_amount_paid_not_exceeds_due"),
    )
...
1. **Invoice Number Format**: INV-YYYYMMDD-XXXX (generated in repository)
2. **Partial Payments**: `amount_paid` tracks cumulative payments
3. **Check Constraints**: Ensure data integrity at database level
4. **Properties**: Calculated fields for business logic
5. **Relationships**: CASCADE delete when policy/invoice is deleted
6. **Stripe Fields**: payment_intent_id and charge_id for integration
7. **Metadata JSONB**: Store webhook data flexibly
8. **Refund Logic**: 30-day window in `is_refundable` property

---

## Import Notes

- Uses existing enums: InvoiceStatus, PaymentStatus, PaymentMethod
- TYPE_CHECKING import for Policy to avoid circular dependency
- All timestamps use datetime.utcnow (consistent with existing code)
